...
<noscript><div class="alert alert-danger text-center m-3">You must enable JavaScript to run this installer.</div></noscript>
...
<div class="installer-card">
    <div class="sidebar">
        <div class="brand-area">
            ...
            <div>
                <h5 class="m-0 fw-bold">Varient&nbsp;<span style="font-size: 14px; color: #52525b">v3.1</span></h5>
    ...
    <div class="content-area">
        ...
        <div id="step-2" class="step-content">
            ...
            <p class="description">
                Enter your purchase code and license key to begin installation.
                If you don't have a license key, create it from the <b><a href="https://codingest.com/licenses" target="_blank">Licenses</a></b> section in the Helpdesk.
                <br><br>
                Don't have a purchase code yet? <b><a href="https://codecanyon.net/item/varient-news-magazine-script/21035226" target="_blank" style="color: #0d6efd; text-decoration: none;">Buy Varient on CodeCanyon</a></b>
            </p>

            <div class="mb-4">
                <label class="form-label">Purchase Code</label>
                <input type="text" class="form-control" name="purchase_code" id="purchaseCode" placeholder="Enter Purchase Code" required>
            </div>
            ...
            <div class="mb-4">
                <div style="padding:16px 18px; border-radius:12px; background:#f8fafc; border:1px solid #e2e8f0; color:#475569; font-size:14px; line-height:1.7;">
                    <strong style="display:block; margin-bottom:6px; color:#0f172a; font-size: 15px;">
                        License Information
                    </strong>
                    Please ensure your license key is entered correctly. If you encounter an <strong>'Invalid License'</strong> error after installation, you can manually
                    update your purchase code and license key directly in the <strong>.env</strong> file located in your root directory.
                </div>
            </div>
        ...
        <div id="step-4" class="step-content">
            ...
            <p class="description">Please ensure the following folders are writable (CHMOD 775).</p>
...
<script>
    ...
    let currentStep = 1;
    const totalSteps = 7;
    let installerData = {}; // Replaces server-side sessions
    ...
    function prevStep(targetStep) {
        ...
        // Skip Step 2 if license was auto-verified (Localhost)
    ...
    function startInstallation() {
        ...
        // Auto-check for localhost
        postData('license', {license_key: 'auto-check'})
            .then(res => {
                btn.disabled = false;
                ...
                if (res.status === 'success') {
                    ...
                    // Provide dummy data so the .env file can still be generated later
                    installerData.license_key = 'localhost-license';
                    installerData.purchase_code = '1234';
    ...
    async function verifyLicense() {
        // Remove ALL spaces (start, end, and middle) using regex
        const purchaseCode = document.getElementById('purchaseCode').value.replace(/\s+/g, '');
        const key = document.getElementById('licenseKey').value.replace(/\s+/g, '');
        const errorBox = document.getElementById('licenseError');

        // Update the input fields so the user sees the spaces were removed
        document.getElementById('purchaseCode').value = purchaseCode;
        document.getElementById('licenseKey').value = key;

        if (!purchaseCode) {
            errorBox.textContent = "Please enter your purchase code.";
        ...
        btnLoad('btnVerify', true, 'Processing...');
        errorBox.classList.add('d-none');

        // Pass both keys to the API
        const result = await postData('license', {
            license_key: key,
        ...
        if (result.status === 'success') {
            // Store both in the local JS state for the final installation step
            installerData.license_key = key;
            installerData.purchase_code = purchaseCode;
    ...
    async function testDatabase() {
        ...
        if (testResult.status === 'success') {
            ...
            // Merge DB data into global state to use in final install instead of sessions
            Object.assign(installerData, data);
    ...
    async function finalInstall() {
        ...
        // Combine DB, License, and Admin Data
        const requestData = {...installerData, ...data};

        btnLoad('btnInstall', true, 'Installing...');
        errorBox.classList.add('d-none');

        const result = await postData('create_admin', requestData);
